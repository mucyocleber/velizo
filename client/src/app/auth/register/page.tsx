    ...
    if (activeTab === 'register') {
      ...
      try {
        ...
        const res = await fetch('/api/auth/register', {
          ...
          body: JSON.stringify({ email: trimmedEmail, password, fullName, role: 'candidate' })
        });
      ...
      <div className="w-full lg:w-[45%] flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 md:p-10 bg-[#F8FAFC] relative dot-grid gap-6">
        ...
        <div className="w-full max-w-md bg-white border border-slate-200/80 p-6 sm:p-8 md:p-10 rounded-[28px] shadow-[0_20px_50px_rgba(15,23,42,0.04)] hover:border-slate-300/60 transition-colors relative z-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {activeTab === 'register' && (
              <div>
                <label className="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider text-center">Full Name</label>
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-400 pointer-events-none">
                    <User className="h-4 w-4" />
                  </span>
                  <input type="text" placeholder="John Doe" className="input-style pl-11 pr-11 text-center text-xs focus:ring-2 focus:ring-blue-100" value={fullName} onChange={(e) => setFullName(e.target.value)} required />
                </div>
              </div>
            )}

            <div>
              <label className="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider text-center">Email Address</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-400 pointer-events-none">
                  <Mail className="h-4 w-4" />
                </span>
                <input type="email" placeholder="name@domain.com" className="input-style pl-11 pr-11 text-center text-xs focus:ring-2 focus:ring-blue-100" value={email} onChange={(e) => setEmail(e.target.value)} required />
              </div>
            </div>

            <div>
              <label className="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider text-center">Password</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-400 pointer-events-none">
                ...
                <input
                  ...
                  minLength={8}
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600 transition-colors cursor-pointer z-20">
            ...
            <button type="submit" disabled={loading} className="w-full py-3 rounded-xl text-xs font-bold tracking-wider uppercase text-white bg-primary hover:bg-[#084e96] transition-all flex items-center justify-center gap-2 shadow-md hover:shadow-lg disabled:opacity-50 mt-6 cursor-pointer">
          ...
          <button type="button" onClick={handleGoogleOAuth} className="w-full py-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold transition-all flex items-center justify-center gap-2.5 shadow-sm hover:shadow-md cursor-pointer">
