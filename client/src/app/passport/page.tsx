...
export default function PassportPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col pb-20 md:pb-8">
      <Header />
      ...
      <div className="md:hidden flex flex-col">
        {/* Cover Banner */}
        <div className="relative w-full h-36 bg-gradient-to-br from-[#0b1329] via-[#0a5fcc] to-indigo-750 overflow-hidden">
          {profile?.banner_url ? (
            ...
            <>
              ...
              <div className="absolute bottom-2 left-4 opacity-10 w-24 h-24 rounded-full bg-white blur-2xl" />
              <Sparkles className="absolute bottom-4 right-4 h-6 w-6 text-white/25" />
            </>
          )}
          {/* Edit Cover Trigger */}
          <button 
            ...
            className="absolute top-3 right-3 bg-black/40 hover:bg-black/60 backdrop-blur-md text-white rounded-xl px-2.5 py-1.5 flex items-center gap-1 text-[9px] font-black transition-all cursor-pointer shadow-sm"
          >
            {uploading === 'banner' ? (
        ...
        <div className="px-4 mb-4">
          {/* Avatar overlapped over cover */}
          <div className="relative w-20 h-20 -mt-10 mb-3 shrink-0">
            <div 
              className="w-full h-full rounded-2xl border-4 border-slate-100 shadow-md bg-gradient-to-br from-[#0a5fcc] to-indigo-600 flex items-center justify-center text-white text-2xl font-black overflow-hidden cursor-pointer" 
            ...
            <button 
              onClick={() => setAvatarMenuOpen(!avatarMenuOpen)} 
              className="absolute -bottom-1 -right-1 h-6.5 w-6.5 bg-white border border-slate-200 rounded-full flex items-center justify-center shadow-md cursor-pointer hover:bg-slate-50 transition-colors"
            >
            ...
            {avatarMenuOpen && (
              <AvatarMenu 
                ...
                onView={() => profile?.avatar_url && setLightboxSrc(profile.avatar_url)} 
          ...
          {/* User Names & Details stacked vertically to give maximum horizontal space */}
          <div className="space-y-1 text-left">
            <h1 className="text-xl font-black text-slate-900 leading-tight">{profile?.full_name}</h1>
            
            <div className="flex flex-wrap items-center gap-2 pt-0.5">
              <span className="text-[9px] font-black uppercase tracking-wider text-[#0a5fcc] bg-blue-50 px-2 py-0.5 rounded border border-blue-100 inline-block">
                {profile?.role === 'candidate' ? 'Job Seeker' : profile?.role || 'Employer'}
              </span>
              {isCandidate && (
                <span className="text-[9px] font-black text-teal-600 bg-teal-50 border border-teal-100 px-2 py-0.5 rounded uppercase tracking-wider">
                  Score: {passport?.career_score ?? 0} pts
                </span>
              )}
            </div>

            <div className="pt-2.5 space-y-1.5 text-xs text-slate-500 font-semibold border-t border-slate-200/60 mt-3">
              <div className="flex items-center gap-2">
                <Mail className="h-4 w-4 text-slate-400 shrink-0" />
                <span className="truncate text-slate-700">{profile?.email}</span>
              </div>
              <div className="flex items-center gap-2">
                <Clock className="h-4 w-4 text-slate-400 shrink-0" />
                <span className="text-slate-500">Joined {joinedDate}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="hidden md:block">
        <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            {/* Cover image banner */}
            <div className="relative w-full h-56 lg:h-64 bg-gradient-to-br from-[#0b1329] via-[#0a5fcc] to-indigo-750 group">
              {profile?.banner_url ? (
                <img src={profile.banner_url} alt="Cover" className="w-full h-full object-cover animate-fade-in" />
              ) : (
                <>
                  ...
                  <Sparkles className="absolute bottom-8 right-12 h-12 w-12 text-white/15" />
                  <p className="absolute bottom-5 left-6 text-white/30 text-[10px] font-black uppercase tracking-widest">VELIZO · Professional Network</p>
                </>
              )}
              {/* Edit Cover Button */}
              <button 
                onClick={() => bannerInputRef.current?.click()} 
                disabled={uploading === 'banner'} 
                className="absolute bottom-4 right-4 bg-black/40 hover:bg-black/60 backdrop-blur-sm text-white rounded-xl px-4 py-2 flex items-center gap-2 text-xs font-black transition-all cursor-pointer opacity-0 group-hover:opacity-100 shadow"
              >
                {uploading === 'banner' ? (
            ...
            {/* Profile Overview Card (Overlapping Cover) */}
            <div className="px-8 pb-6 relative">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                
                {/* Left Side: Avatar + Names Column */}
                <div className="flex items-end gap-5">
                  {/* Avatar Frame overlapping cover image */}
                  <div className="relative shrink-0 -mt-16 z-10">
                    <div 
                      className="h-28 w-28 rounded-3xl border-4 border-white shadow-lg bg-gradient-to-br from-[#0a5fcc] to-indigo-600 flex items-center justify-center text-white text-3xl font-black overflow-hidden cursor-pointer"
                      onClick={() => setAvatarMenuOpen(!avatarMenuOpen)}
                    >
                      {uploading === 'avatar' ? (
                    ...
                    <button 
                      onClick={() => setAvatarMenuOpen(!avatarMenuOpen)} 
                      className="absolute -bottom-1.5 -right-1.5 h-7 w-7 bg-white border border-slate-200 rounded-full flex items-center justify-center shadow-md cursor-pointer hover:bg-slate-50 transition-colors"
                    >
                  ...
                  <div className="space-y-1 mb-2">
                    ...
                    <div className="flex items-center gap-2 pt-0.5">
                      <span className="text-[10px] font-black uppercase tracking-wider text-[#0a5fcc] bg-blue-50 px-2.5 py-0.5 rounded border border-blue-100">
                        {profile?.role === 'candidate' ? 'Job Seeker' : profile?.role || 'Employer'}
                      </span>
                      <span className="flex items-center gap-1 text-[11px] text-slate-400 font-semibold">
                        <Clock className="h-3.5 w-3.5 shrink-0" /> Joined {joinedDate}
                      </span>
                    </div>
                ...
                {/* Right Side: Trust / Score Badge */}
                {isCandidate && (
                  <div className="bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-100 rounded-2xl p-4 text-center shrink-0 min-w-[140px] shadow-3xs mb-2">
                    <span className="text-[9px] font-black text-teal-655 uppercase tracking-widest block">Passport Score</span>
                    <span className="text-3xl font-black text-teal-500 leading-none mt-1 block">{passport?.career_score ?? 0}</span>
                    <span className="text-[9px] text-teal-400 font-bold mt-1 block">out of 100</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-6 items-start">
            ...
            <div className="col-span-1 space-y-4">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-3">
                <h3 className="text-[10px] font-black text-slate-450 uppercase tracking-widest border-b border-slate-100 pb-2">Profile info</h3>
                
                <div className="space-y-3 text-[11px] text-slate-600 font-semibold">
                  <div className="space-y-0.5">
                    <span className="text-[9px] text-slate-400 block font-bold uppercase tracking-wider">Email Address</span>
                    <span className="text-slate-800 break-all">{profile?.email}</span>
                  </div>
                  <div className="space-y-0.5">
                    <span className="text-[9px] text-slate-400 block font-bold uppercase tracking-wider">Member Since</span>
                    <span className="text-slate-800">{joinedDate}</span>
                  </div>
                  <div className="space-y-0.5">
                    <span className="text-[9px] text-slate-400 block font-bold uppercase tracking-wider">Portal Role</span>
                    <span className="text-slate-800 capitalize">{profile?.role} Portal</span>
                  </div>
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full py-3.5 bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 text-[10px] font-black uppercase tracking-wider rounded-2xl transition-all flex items-center justify-center gap-2 cursor-pointer shadow-3xs"
              >
